...
# *Comments from Code Reviewer (Shawn)* ####
  ...
  # 7. Line 23: Good work with using nchar() to check that the input has only 3 digits; this wasn't explicitly asked for so you went above and beyond with this one
  # 8. Line 24:  It might be interesting to add a line of code to check for positive numbers (as.numeric(user_input) > 0)
  # 9. Line 25: To avoid ambiguity, the wording in the comment can be tweaked to "each *digit* from the input" as the input itself itself is supposed to be a number
  ...
  # 11. Line 26: It seems that there is a bug with the condition in the if statement (none of the error messages you created appear) if we input a negative two-digit number (say, -10).
      # The reason this happens is because right now none of the validity checks you have seem to catch it: -10 passes checks for (1) and (2) as it is an integer; it also passes the check for (3) as the nchar() function sees it as a 3-digit character.
      ...
      # To resolve this bug, you can add the test for positive numbers (please see my 8th comment);
